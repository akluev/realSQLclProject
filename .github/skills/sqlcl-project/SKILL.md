---
name: sqlcl-project
description: 'Oracle SQLcl Project deployment pipeline skill. Use when: project export, project stage, project stage add-custom, DML file creation, changeset splitting, Liquibase changesets, deploy install.sql, stage.changelog.xml, branch immutability, MCP SQLcl connection, seed data, lookup table DML, APEX workspace setup via SQLcl, APEXlang compile apex import, APEXlang validate apex validate, prj_compile, prj_validate.'
---
...
```
1. Call: mcp_sqlcl_local_connect(connection_name: "proj_vm", model: "Claude Sonnet 4.5")
...
- **Set current_schema before queries** — Always `alter session set current_schema = demo1` before SELECT queries

### Example Pattern

```javascript
// 1. Connect (always)
mcp_sqlcl_local_connect(connection_name: "proj_vm", model: "Claude Sonnet 4.5")
...
mcp_sqlcl_local_run-sql(
  ...
  sql: "alter session set current_schema = demo1;\nselect * from job"
)
```
...
## Workflow 5: APEXlang Compile and Validate

Use when the user asks to **compile**, **import**, or **validate** an APEX app from its APEXlang (`.apx`) source files.

> **Version requirement:** `apex import` / `apex validate` require SQLcl ≥ 26.1.2 (`_sqlplus_release >= 2601020000`). Use the version check from Workflow 2. If the version is lower, tell the user to upgrade and stop.

### Step 1 — Resolve App Details via MCP (ALWAYS REQUIRED)

**Never guess the path.** The workspace name cannot be inferred locally, and the alias may differ from the filesystem folder name if it was renamed in APEX. Always query MCP first.

Connect and run:
```sql
select application_id,
       lower(owner)                                                                        as schema_lc,
       lower(alias)                                                                        as alias_lc,
       workspace,
       'src/database/'||lower(owner)||'/apex_apps/f'||application_id||'/'||lower(alias)   as input_path
  from apex_applications
 where application_id = <app_id>;
```

**If the query returns no rows:**
> ❌ Application `<app_id>` was not found in `apex_applications`. Please check the application ID and confirm you are connected to the correct environment.

Stop — do not proceed.

**If the query returns a row**, extract `schema_lc`, `alias_lc`, `workspace`, and `input_path` from the result.

---

### Step 2 — Compile: `apex import`

Compiles and imports APEXlang source into the APEX instance.

> ⚠️ **MCP limitation — do not run `apex import` via MCP.**  
> The MCP server creates connections with JDBC-level autocommit ON. `apex import` requires JDBC-level autocommit OFF to be able to roll back on error. SQLcl's `set autocommit off` / `show autocommit` only controls the SQLcl script layer — it does not affect the JDBC connection property. The command will always fail with `ORA-17274` via MCP regardless of the SQLcl setting.  
> **Present the resolved command to the user and ask them to run it in their local SQLcl session.**

Command to give the user:
```sqlcl
apex import -input src/database/<schema_lc>/apex_apps/f<app_id>/<alias_lc> -ws <workspace>
```

Example for app 106:
```sqlcl
apex import -input src/database/demo1/apex_apps/f106/alias -ws DEMO1
```

---

### Step 2 (alternative) — Validate: `apex validate`

Validates APEXlang source without importing to the database. Useful for syntax/schema checks before committing.

✅ **`apex validate` works via MCP** — it is a read-only operation and does not require JDBC-level autocommit OFF. Run directly:

```sqlcl
apex validate -input src/database/<schema_lc>/apex_apps/f<app_id>/<alias_lc> -ws <workspace>
```

---

### SQLcl Aliases: `prj_compile` and `prj_validate`

The `prj_compile` and `prj_validate` aliases let users run compile/validate directly from a SQLcl session without the agent. They query `apex_applications` for `owner`, `alias`, and `workspace`, build the source path, then run the appropriate command.
...
**Load the skill alias bundle into a SQLcl session:**
```sqlcl
alias load <path-to-skill>/sqlcl-project-aliases.xml
```

**Persist for future sessions (run once after loading):**
```sqlcl
alias save
```

The alias definitions are bundled in `sqlcl-project-aliases.xml` alongside this skill file.
