- [2. Enhancement Requests](#2-enhancement-requests)
  - [New SQLcl 26.1.2 / APEXlang Enhancement Requests](#new-sqlcl-2612--apexlang-enhancement-requests)
  - [🟥 P0 — Critical (workflow or correctness impact)](#-p0--critical-workflow-or-correctness-impact)
  ...
  - [🟧 P1 — High (major workflow pain, manual workarounds exist)](#-p1--high-major-workflow-pain-manual-workarounds-exist)
    - [2.2. Ability to Set the “Change” Name Inside the Branch](#22-ability-to-set-the-change-name-inside-the-branch)
      - [Problem](#problem)
      - [Suggested Enhancements](#suggested-enhancements)
      - [Goal](#goal)
    - [2.3. Only Append, Don’t Rewrite `stage.changelog.xml`](#23-only-append-dont-rewrite-stagechangelogxml)
    ...
    - [2.5. Add a `context` per Changeset (for precise manual sync)](#25-add-a-context-per-changeset-for-precise-manual-sync)
      ...
      - [Why `mark-next-changeset-ran` isn’t enough](#why-mark-next-changeset-ran-isnt-enough)
      - [Expected vs. Actual Behavior](#expected-vs-actual-behavior)
      - [Recommendation](#recommendation)
  - [🟨 P2 — Medium (quality-of-life / developer productivity)](#-p2--medium-quality-of-life--developer-productivity)
...
## New SQLcl 26.1.2 / APEXlang Enhancement Requests

These are newer SQLcl 26.1.2 / APEXlang enhancement requests captured while testing APEX 26.1 with SQLcl Project. They are described in more detail in [14. SQLcl Project with APEXlang: First Impressions](14.-SQLcl-Project-with-APEXlang-First-Impressions.md).

| Area | Enhancement Request | Forum Post |
|---|---|---|
| `skills sync` | Support repository-local installs and granular subskill sync, such as `apex/apexlang` or `db/sqlcl`, while preserving the namespace in the target skill folder. | [SQLcl skills sync should support repository-local installs and granular subskill sync](https://forums.oracle.com/ords/apexds/post/sqlcl-skills-sync-should-support-repository-local-installs-2681) |
| APEXlang export/import | Make APEXlang export, validate, and import path behavior symmetric, so the directory used for export can also be used for validate/import when the APEXlang root can be resolved unambiguously. | [Make APEXlang export, validate, and import path behavior symmetric](https://forums.oracle.com/ords/apexds/post/make-apexlang-export-validate-and-import-path-behavior-symm-7607) |
