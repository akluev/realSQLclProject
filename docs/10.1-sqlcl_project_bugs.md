- [1. Bugs](#1-bugs)
  - [New SQLcl 26.2 Bugs](#new-sqlcl-262-bugs)
  - [New SQLcl 26.1.2 / APEXlang Bugs](#new-sqlcl-2612--apexlang-bugs)
  ...
  - [P2 — Medium](#p2--medium)
    ...
    - [1.8. `-o` Option of `project export` Does Not Export Synonyms](#18--o-option-of-project-export-does-not-export-synonyms)
      - [Problem](#problem)
...
These issues were encountered while evaluating SQLcl 26.2 with an existing SQLcl Project.

| Area | Issue | Workaround | Forum Post |
|---|---|---|---|
| `project export` | SQLcl 26.2 exports table DML and user definitions that were not requested. | Add `export_type != 'TABLES_DML'` and `export_type != 'ALL_USERS'` to `.dbtools/filters/project.filters`. | [APEX application project export is exporting application DML data and users](https://forums.oracle.com/ords/apexds/post/apex-app-project-export-is-exporting-application-dml-data-a-4157) |

---

## New SQLcl 26.1.2 / APEXlang Bugs

These are newer SQLcl 26.1.2 / APEXlang issues captured while testing APEX 26.1 with SQLcl Project. They are described in more detail in [14. SQLcl Project with APEXlang: First Impressions](14.-SQLcl-Project-with-APEXlang-First-Impressions.md).

| Area | Issue | Forum Post |
|---|---|---|
| `project export` / ORDS | When connected as a DBA-like deployer account that is not REST-enabled, SQLcl exports ORDS metadata for the connected user instead of the configured project schemas. The ORDS export is silently skipped; `ORA-20012: Schema not REST enabled` is visible only with debug output. | [SQLcl Project ORDS export should use configured project schemas](https://forums.oracle.com/ords/apexds/post/sqlcl-project-ords-export-should-use-configured-project-sch-9778) |
| `project export` | SQLcl corrupts APEX static files during APEXlang export. | [SQLcl corrupts APEX static files during export (APEXLang)](https://forums.oracle.com/ords/apexds/post/sqlcl-corrupts-apex-static-files-during-export-apexlang-7800) |
| `project export` | SQLcl Project export does not remove stale APEX alias folders when the application alias changes. | [SQLcl Project export should remove stale APEX alias folders](https://forums.oracle.com/ords/apexds/post/sqlcl-project-export-should-remove-stale-apex-alias-folders-6627) |
| APEXlang export | In APEX 26.1, `apex_export.get_application` with `p_type => 'APEXLANG'` ignores `p_with_supporting_objects => 'N'` and exports Supporting Objects regardless of the application's **Include in Export** setting. | [Export to APEXlang ignores `p_with_supporting_objects`](https://forums.oracle.com/ords/apexds/post/export-to-apexlang-ignores-p-with-supporting-objects-and-al-8324) |
| `project stage` / APEXlang | **Resolved in SQLcl 26.2.** In SQLcl 26.1, APEX Supporting Object `.sql` files exported by earlier versions are incorrectly analyzed as hard objects with hash mismatches, causing `project stage` to fail with unrecoverable hash issues. | [PROJECT STAGE fails when analyzing Supporting Objects](https://forums.oracle.com/ords/apexds/post/export-to-apexlang-ignores-p-with-supporting-objects-and-al-9749) |
| SQLcl MCP / `apex import` | `apex import` fails through SQLcl MCP with `ORA-17274: Could not rollback with auto-commit enabled.` | [APEXLang IMPORT fails via SQLcl MCP](https://forums.oracle.com/ords/apexds/post/apexlang-import-fails-via-sqlcl-mcp-with-error-ora-17274-co-5766) |
| `project export` / APEXlang | `project export` lowercases all APEXlang file and folder names, whereas the APEX export API and `apex export` command preserve the original casing (e.g., camelCase). This makes component names harder to read and navigate in editors, and creates unnecessary differences between export methods. | [SQLcl project export lowercases APEXlang file and folder names](https://forums.oracle.com/ords/apexds/post/sqlcl-project-export-lowercases-apexlang-file-and-folder-na-5571) |
...
### 1.8. `-o` Option of `project export` Does Not Export Synonyms

**Observed in:** SQLcl 26.1

The `-o` option of the `project export` command silently ignores synonyms — it exports 0 objects even when the synonym exists and is correctly exported by `project export` without the `-o` filter.

**Steps to reproduce:**

Set `project.filters` to include synonyms (optional, but makes the test cleaner):
```
export_type in ( 'ALL_OBJECTS', 'ALL_SYNONYMS'),
```

Create a synonym:
```sql
create table demo2.test_syn as select * from demo2.emp;
grant select on demo2.test_syn to demo1;
create synonym demo1.test_syn for demo2.test_syn;
```

Run export with and without `-o`:
```
SQL> project export -v
DEMO1.TEST_SYN
-------------------------------
SYNONYM                       1
-------------------------------
...
SQL> project export -o test_syn -v
-------------------------------
-------------------------------
Exported 0 objects
Elapsed 0 sec
...
`project export` without arguments correctly exports the synonym; `project export -o test_syn` exports nothing.

Based on the debug output, synonyms appear to be selected differently from other object types internally. The `-o` filter logic does not apply to that alternate selection path, so synonym names are never matched.

**Expected vs. Actual Behavior**

| Aspect | Expected | Actual |
|---------|-----------|--------|
| `project export -o <synonym_name>` | Exports the named synonym | Exports 0 objects |
| Behavior consistency | `-o` works the same for all object types | Synonyms are silently skipped |

**Forum post:** [SQLcl Project: Synonyms are not exported with `-o` switch](https://forums.oracle.com/ords/apexds/post/sqlcl-project-synonyms-are-not-exported-with-o-switch-0194)
