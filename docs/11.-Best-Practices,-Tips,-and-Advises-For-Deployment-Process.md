- [1. Use Secrets and Environment-Specific Parameters](#1-use-secrets-and-environment-specific-parameters)
  - [1.1. Use ${VAR\_NAME} syntax](#11-use-var_name-syntax)
  - [1.2. Setting Environment Variables](#12-setting-environment-variables)
    - [1.2.1. Unix / Linux / macOS](#121-unix--linux--macos)
    - [1.2.2. Windows PowerShell](#122-windows-powershell)
    - [1.2.3. Windows CMD](#123-windows-cmd)
  - [1.3. ⚠ Important Warning](#13--important-warning)
  - [1.4. Validating Environment Variables in Changesets (Best Practice)](#14-validating-environment-variables-in-changesets-best-practice)
- [2. Working with Multi-DDL-Statements Changesets](#2-working-with-multi-ddl-statements-changesets)
...
- [9. Dropping and Recreating Constraint](#9-dropping-and-recreating-constraint)
- [10. Refresh SQLcl Version](#10-refresh-sqlcl-version)
- [11. XML Changesets for runApexScript and SQLcl @/@@ Calls](#11-xml-changesets-for-runapexscript-and-sqlcl--calls)
...
## 1.4. Validating Environment Variables in Changesets (Best Practice)
...
# 2. Working with Multi-DDL-Statements Changesets
...
>Note: Use alias `prj_exp_app app_id`  for this task  
...
# 9. Dropping and Recreating Constraint 

Use the technique below to drop and recreate a constraint, for example to change `on delete` from restrict to cascade or change  enable/disable validate/novalidate. Add conditions to drop as the contraint may not exist in all environments 

```sql
-- liquibase formatted sql
-- sqlcl_snapshot src/database/cla_apex/ref_constraints/sub_aircraft_fk.sql:ad0a27eef3fcdf3e2b7cd9a23717466ebdf7f5f4:2859202208434b9fd806e38350af50a71731fa3b:alter


-- changeset CLA_APEX:1764290961077.1 stripComments:false  logicalFilePath:hotfix-drift-27-nov-25\cla_apex\ref_constraints\sub_aircraft_fk.sql
--preconditions onFail:MARK_RAN 
--precondition-sql-check expectedResult:1 SELECT count(*) from ALL_CONSTRAINTS where constraint_name ='SUB_AIRCRAFT_FK' and owner='CLA_APEX'


alter table cla_apex.sub_subscriptions
    drop constraint sub_aircraft_fk
       ;


-- changeset CLA_APEX:1764290961077.2 stripComments:false  logicalFilePath:hotfix-drift-27-nov-25\cla_apex\ref_constraints\sub_aircraft_fk.sql

alter table cla_apex.sub_subscriptions
    add constraint sub_aircraft_fk
        foreign key ( aircraft_id )
            references cla_apex.sub_aircraft ( id )
            enable novalidate;

```

# 10. Refresh SQLcl Version 

Script [refresh_sqlcl.sh](../scripts/bash/tools/refresh_sqlcl.sh) installs the latest available version of Oracle SQLcl on Windows using Git Bash.

Instead of hardcoding a version number, it downloads SQLcl via Oracle’s official sqlcl-latest.zip alias, ensuring the most recent release is always used. The script preserves the previous installation by moving it to sqlcl-latest-old, extracts the new version into sqlcl-latest-new, and then updates a symbolic link (sqlcl-latest) to point to the new installation.

This directory-and-symlink strategy provides a safe, repeatable upgrade mechanism that works reliably in Git Bash, avoids partial overwrites, and allows easy rollback if needed.


# 11. XML Changesets for runApexScript and SQLcl @/@@ Calls

If a script calls other scripts via SQLcl `@` or `@@`, you must wrap it in an **XML changeset** using `runApexScript`. A real example is:
...
<changeSet id="SqlCl:1769116579255.1" author="SQLCL-Generated"  failOnError="true"   runOnChange="true" logicalFilePath="aop_upgrade_25.2/_custom/aop_install.xml">
    <n0:runApexScript objectName="install" objectType="SCRIPT" ownerName="CLA_DEPLOYER" sourceType="STRING"  relativeToChangelogFile="true" >
        <n0:source><![CDATA[

-- liquibase formatted sql
-- changeset   stripComments:false logicalFilePath:aop_upgrade_25.2\_custom\aop_install.sql
-- sqlcl_snapshot dist\releases\next\changes\aop_upgrade_25.2\_custom\aop_install.sql:null:null:custom

prompt Install AOP Packages into CLA_UTILITIES schema

alter session set current_schema = CLA_UTILITIES;
...
-- spool off
alter session set current_schema = CLA_DEPLOYER;
...
This XML changeset installs the third-party AOP server and calls other scripts, so plain SQL changesets are not enough.

**Critical:** Always set `logicalFilePath` and `relativeToChangelogFile="true"` in the XML changeset:

```xml
<changeSet author="SqlCl" id="INSTALL_AOP_25_2" logicalFilePath="aop_upgrade_25.2/_custom/aop_install.xml">
  <runApexScript relativeToChangelogFile="true" runOnChange="true" runAlways="false">
    <script>_custom/aop_install.sql</script>
  </runApexScript>
</changeSet>
```

Why this matters:
- Without `logicalFilePath` and `relativeToChangelogFile="true"`, the filename is calculated as a relative path from the master SQLcl Project changeset, `dist/releases/main.changelog.xml`. Before `project release -version 26.2` it becomes `releases/next/changes/aop_upgrade_25.2/_custom/aop_install.xml`, and after release it becomes `releases/26.2/changes/aop_upgrade_25.2/_custom/aop_install.xml`. Liquibase treats that as a *new* changeset and wants to run it again.
- Setting `logicalFilePath="aop_upgrade_25.2/_custom/aop_install.xml"` prevents the path from changing between `releases/next` and a versioned release.

**If you already ran an XML changeset without `logicalFilePath`:** you can "hack" Liquibase:
1. Set `logicalFilePath` in the XML file as described above.
2. Update the filename in `DATABASECHANGELOG` so it matches the new logical path in the file. Make sure `id` and `author` match the values from your XML changeset:

```sql
update databasechangelog
   set filename = 'aop_upgrade_25.2/_custom/aop_install.xml'
 where id     = 'SqlCl:1769116579255.1'
   and author = 'SQLCL-Generated'
   and filename like '%releases/next/changes/aop_upgrade_25.2/_custom/aop_install.xml';
